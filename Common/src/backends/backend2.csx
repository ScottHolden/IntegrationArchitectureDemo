...
public static async Task<IActionResult> Run(HttpRequest req, ILogger log)
{
    try
    {
        ...
        log.LogInformation($"Backend 2 - Processing address change for user {change.UserId}");
        await Task.Delay(15000);
        log.LogInformation($"Backend 2 - Address updated for user {change.UserId}");
        return new OkObjectResult("Backend 2 - Slow - Ok");
    }
    catch (Exception e)
    {
